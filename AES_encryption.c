...
int main() { 
    ...
    int row, col; 
    int round1 = 0; 
    int m[4][4] = {{0x02, 0x03, 0x01, 0x01}, {0x01, 0x02, 0x03, 0x01}, {0x01, 0x01, 0x02, 
0x03}, {0x03, 0x01, 0x01, 0x02}}; 
    int sbox[16][16] = { 
...
0x54, 0xBB, 0x16}}; 
    ...
    key[32] = '\0'; 
    for (int i = 0; i < 16; i++) { 
        char temp[] = {key[2 * i], key[(2 * i) + 1], '\0'}; 
        int value = (int)strtol(temp, NULL, 16); 
        a1[i / 4][i % 4] = value; 
    } 
    for (int i = 0; i < 16; i++) { 
        char temp[] = {pt[2 * i], pt[(2 * i) + 1], '\0'}; 
        int value = (int)strtol(temp, NULL, 16); 
        s[i % 4][i / 4] = value; 
    } 
    for (int i = 4; i < 44; i++) { 
        if (i % 4 == 0) { 
            ...
            sub[0] = sub[0] ^ rcon[round1]; 
            round1++; 
            for (int j = 0; j < 4; j++) { 
                a1[i][j] = sub[j] ^ a1[i - 4][j]; 
            } 
        } else { 
            for (int j = 0; j < 4; j++) { 
                a1[i][j] = a1[i - 1][j] ^ a1[i - 4][j]; 
            } 
        } 
    } 
    for (int i = 0; i < 4; i++) { 
